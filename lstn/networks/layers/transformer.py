...
class LongShortTermTransformerBlock(nn.Module):
    ...
    def forward(self, tgt, long_term_memory=None, short_term_memory=None, curr_id_emb=None,
        ...
        # Self-attention # 下面五行是非常标准的self-attention的计算
        ...
        if curr_id_emb is not None:
            global_K, global_V = self.fuse_key_value_id(curr_K, curr_V, curr_id_emb)  # key不变，V+curr_id_emb并通过linear
        ...
        tgt2 = self.long_term_attn(curr_Q, global_K, global_V)[0]  # [900, 2, 256] # 在第一次进行long term时等价于self-attention
