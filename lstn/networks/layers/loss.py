...
def flatten_probas(probas, labels, ignore=255):
    ...
    """
    B, C, H, W = probas.size()  # [1, 2, 465, 465]
    probas = probas.permute(0, 2, 3, 1).contiguous().view(-1, C)  # [465*645, 2]
    labels = labels.view(-1)  # [465*465]
    if ignore is None:
...
class DiceLoss(nn.Module):
    ...
    def forward(self, tmp_dic, label_dic, step=None):
        ...
        for idx in range(len(tmp_dic)):
            ...
            loss = dice_loss(*flatten_probas(pred, label, ignore=self.ignore_index))
...
class SoftJaccordLoss(nn.Module):
    ...
    def forward(self, tmp_dic, label_dic, step=None):
        total_loss = []
        for idx in range(len(tmp_dic)):
            pred = tmp_dic[idx]  # [1, 2, 465, 465]
            label = label_dic[idx]  # [1, 465, 465]
            pred = F.softmax(pred, dim=1)
            label = label.view(1, 1, pred.size()[2], pred.size()[3])
            loss = tversky_loss(*flatten_probas(pred, label, ignore=self.ignore_index), alpha=1.0, beta=1.0)
            total_loss.append(loss.unsqueeze(0))
        total_loss = torch.cat(total_loss, dim=0)
        return total_loss
...
class CrossEntropyLoss(nn.Module):
    ...
    def forward(self, dic_tmp, y, step):
        ...
        for i in range(len(dic_tmp)):
            pred_logits = dic_tmp[i]  # [1, obj_num+1, 465, 465]
            gts = y[i]  # [1, 456, 456]
            if self.top_k_percent_pixels is None:
                ...
                pred_logits = pred_logits.view(-1, pred_logits.size(1), pred_logits.size(2)*pred_logits.size(3))  # [1, obj+1, 465*465]
                gts = gts.view(-1, gts.size(1)*gts.size(2))  # [1, 465*465]
                pixel_losses = self.celoss(pred_logits, gts)  # [1, 465*465]
                if self.hard_example_mining_step == 0:
                    ...
                    top_k_pixels = int((ratio*self.top_k_percent_pixels+(1.0-ratio)) * num_pixels)  # 1-0.85*r   # top_k: 1->0.15
